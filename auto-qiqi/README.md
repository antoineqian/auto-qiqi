...
| **AutoBattleEngine** | Roaming wild battles: scan for Pokemon, walk/aim, simulate send-out. Modes: **OFF**, **BERSERK**, **ROAMING**, **TRAINER**. ROAMING priority: boss (kill) &gt; uncaught (capture) &gt; uncaught legendary in `legendaryKillWhitelist` (kill) &gt; uncaught legendary (capture) &gt; caught legendary in `battleTargetWhitelist` (kill) &gt; caught legendary not in whitelist (recapture) &gt; whitelisted (kill). **Roaming nextleg/afk (when `roamingNextlegAfkEnabled`):** polls `/nextleg` for the single global timer, sends `/afk` periodically for AFK points, and moves the camera shortly before timer expiry (config `roamingCameraMoveSecondsBefore`) to disable AFK and stay eligible for legendary spawn—no world hop. Loot pickup; engage blacklist. When not in capture mode, battle decisions go through **BattleDecisionRouter**. |
...
| **WorldTracker** | Tracks current world name and per-world timer state (from chat). When the server uses a single global timer (nextleg system), **global timer** is updated from `/nextleg` in roaming and stored separately (`getGlobalTimer()`, `getGlobalRemainingSeconds()`). `getSoonestRemainingSeconds()` returns the soonest event time across all worlds (used for boss-priority rule). |
