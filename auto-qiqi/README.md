...
| Class | Role |
|-------|------|
| **AutoSwitchEngine** | State machine for legendary lifecycle: IDLE → open menu → switch world (GUI or `/home`) → poll timer (`/nextleg`) → wait for response. On “legendary appeared” event, switches to event world. Can pause for capture (`PAUSED_FOR_CAPTURE`) and auto-resume after capture + min pause time. **Never teleports or opens the world menu while in a Cobblemon battle:** each tick re-checks battle and aborts to IDLE (and cancels any pending command) if the player entered battle during a switch flow. |
...
| **AutoQiqiHud** | Renders battle HUD (top-center: opponent + best matchup suggestion, then **trainer planned action** — what trainer mode would do this turn: FIGHT → move or SWITCH → Pokémon); and legendary HUD (timer, state, world list, etc.) via `HudRenderCallback`. |
