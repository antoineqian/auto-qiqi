...
public class AutoSwitchEngine {
    ...
    public void onWorldsLearned() {
        ...
        AutoQiqiClient.logDebug("Legendary", "Worlds learned: " + count + " GUI mapped. " + dumpTimerSummary());
    ...
    public void onWorldSwitchComplete() {
        ...
        if (run.state == LegendaryRunState.State.OPENING_MENU) {
            ...
            if (run.switchPurpose == LegendaryRunState.SwitchPurpose.EVENT) {
                AutoQiqiClient.logDebug("Legendary", "GUI switch complete (EVENT) -> " + run.targetEventWorld);
                ...
                AutoQiqiClient.logDebug("Legendary", "GUI switch complete (POLL) -> " + run.currentPollWorld);
    ...
    public void onGuiTimeout() {
        if (run != null && run.state == LegendaryRunState.State.OPENING_MENU) {
            AutoQiqiClient.logDebug("Legendary", "GUI timeout (purpose=" + run.switchPurpose + ", target=" + run.currentPollWorld + ")");
    ...
    public void pauseForCapture(String pokemonName) {
        ...
        AutoQiqiClient.logDebug("Legendary", "PAUSING for capture: " + pokemonName + " (was " + r.state + ")");
    ...
    public void resumeFromPause() {
        if (run != null && run.state == LegendaryRunState.State.PAUSED_FOR_CAPTURE) {
            AutoQiqiClient.logDebug("Legendary", "RESUMING from capture pause (was paused for " + run.pausedForPokemon + ")");
    ...
    public void tick() {
        ...
        if (!config.legendaryAutoSwitch && r.state != LegendaryRunState.State.IDLE) {
            AutoQiqiClient.logDebug("Legendary", "legendaryAutoSwitch OFF while in " + r.state + ", cancelling");
            GuiWorldSwitcher.get().cancelPending();
        ...
        if (r.state != LegendaryRunState.State.IDLE && r.state != LegendaryRunState.State.PAUSED_FOR_CAPTURE
                && isInBattle()) {
            AutoQiqiClient.logDebug("Legendary", "In battle during " + r.state + " — aborting to IDLE (no teleport)");
            GuiWorldSwitcher.get().cancelPending();
            r.pendingCommand = null;
            r.pendingHomeWorld = null;
            r.isHomeTeleport = false;
    ...
    private void handleIdle(MinecraftClient client, AutoQiqiConfig config, LegendaryRunState r) {
        ...
        if (isBossActive() && shouldPrioritizeBossOverLegendary()) {
            if (r.bossYieldStartTick == 0) {
                ...
                AutoQiqiClient.logDebug("Legendary", "IDLE: yielding to boss encounter (timer >= " + BOSS_PRIORITY_MIN_REMAINING_SECONDS + "s)");
            }
            long waited = tickCount - r.bossYieldStartTick;
            if (waited >= BOSS_YIELD_TIMEOUT_TICKS) {
                AutoQiqiClient.logDebug("Legendary", "IDLE: boss yield timeout after " + (waited / 20) + "s, proceeding anyway");
                r.bossYieldStartTick = 0;
            } else {
                if (waited % 200 == 0 && waited > 0) {
                    AutoQiqiClient.logDebug("Legendary", "IDLE: still yielding to boss (" + (waited / 20) + "s / " + (BOSS_YIELD_TIMEOUT_TICKS / 20) + "s)");
                }
                return;
            }
        } else {
            if (isBossActive() && !shouldPrioritizeBossOverLegendary()) {
                AutoQiqiClient.logDebug("Legendary", "IDLE: boss nearby but timer < " + BOSS_PRIORITY_MIN_REMAINING_SECONDS + "s — proceeding with legendary switch");
            }
            r.bossYieldStartTick = 0;
        }
        ...
        if (!switcher.hasLearnedWorlds()) {
            ...
            AutoQiqiClient.logDebug("Legendary", "IDLE: worlds not learned yet, opening /monde for learning");
        ...
        if (currentWorldName != null) {
            ...
            if (cwTimer != null && cwTimer.isTimerKnown()
                ...
                AutoQiqiClient.logDebug("Legendary", "IDLE: current world " + currentWorldName
        ...
        if (eventWorld != null) {
            ...
            AutoQiqiClient.logDebug("Legendary", "IDLE: event switch needed -> " + eventWorld
                    + " (timer=" + evtInfo + ", from=" + currentWorldName + ")");
            if (config.isHomeWorld(eventWorld)) {
                String homeName = config.getHomeCommand(eventWorld);
                AutoQiqiClient.logDebug("Legendary", "EVENT! Home switch -> " + eventWorld + " via /home " + homeName);
                stopActiveEngines();
        ...
        if (expiredWorld != null) {
            ...
            AutoQiqiClient.logDebug("Legendary", "IDLE: repoll expired timer -> " + expiredWorld
        ...
        if (unknownWorld != null) {
            AutoQiqiClient.logDebug("Legendary", "IDLE: discovering unknown timer -> " + unknownWorld
        ...
        if (soonestWorld != null) {
            ...
            AutoQiqiClient.logDebug("Legendary", "IDLE: all timers known, camping soonest -> " + soonestWorld
    ...
    private void switchAndPoll(MinecraftClient client, AutoQiqiConfig config, String worldName, LegendaryRunState r) {
        ...
        if (worldName.equalsIgnoreCase(currentWorld)) {
            AutoQiqiClient.logDebug("Legendary", "switchAndPoll: already in " + worldName + ", polling directly");
            ...
            AutoQiqiClient.logDebug("Legendary", "switchAndPoll: " + currentWorld + " -> " + worldName + " via /home " + homeName);
            com.cobblemoon.autoqiqi.common.SessionLogger.get().logWorldSwitch(currentWorld, worldName);
            stopActiveEngines();
            releaseMovementKeys(client);
            sendCommand(client, "/home " + homeName, r);
            ...
            GuiWorldSwitcher switcher = GuiWorldSwitcher.get();
            if (switcher.hasLearnedWorlds()) {
                AutoQiqiClient.logDebug("Legendary", "switchAndPoll: " + currentWorld + " -> " + worldName + " via GUI");
                com.cobblemoon.autoqiqi.common.SessionLogger.get().logWorldSwitch(currentWorld, worldName);
                ...
                AutoQiqiClient.logDebug("Legendary", "switchAndPoll: can't switch to " + worldName + " - worlds not learned yet");
            }
        }
    }
    ...
    private void stopActiveEngines() {
        ...
        if (walkerActive || captureActive) {
            AutoQiqiClient.logDebug("Legendary", "Stopping active engines: walker=" + walkerActive + " capture=" + captureActive);
        }
    ...
    private void handleOpeningMenu(LegendaryRunState r) {
        if ((tickCount - r.stateEnteredTick) > 400) {
            AutoQiqiClient.logDebug("Legendary", "OPENING_MENU timeout after 400 ticks (purpose=" + r.switchPurpose + ", target=" + r.currentPollWorld + ")");
            GuiWorldSwitcher.get().cancelPending();
    ...
    private void handleWaitingAfterSwitch(MinecraftClient client, LegendaryRunState r) {
        ...
        if ((tickCount - r.stateEnteredTick) > waitTicks) {
            if (r.pendingHomeWorld != null) {
                if (!verifyHomeDimension(client, r.pendingHomeWorld)) {
                    if (r.homeRetryCount < MAX_HOME_RETRIES) {
                        ...
                        AutoQiqiClient.logDebug("Legendary", "WAITING_AFTER_SWITCH: dimension mismatch (movement cancelled /home?), retrying /home " + homeCmd + " (" + r.homeRetryCount + "/" + MAX_HOME_RETRIES + ")");
                        if (client.player != null) {
                        ...
                        AutoQiqiClient.logDebug("Legendary", "WAITING_AFTER_SWITCH: dimension still wrong after " + MAX_HOME_RETRIES + " retries, aborting switch");
                        if (client.player != null) {
                ...
                r.homeRetryCount = 0;
                AutoQiqiClient.logDebug("Legendary", "WAITING_AFTER_SWITCH: /home warmup done, setting currentWorld=" + r.pendingHomeWorld);
            ...
            if (arrivalHome != null && !r.arrivalHomePending) {
                AutoQiqiClient.logDebug("Legendary", "WAITING_AFTER_SWITCH: arrival home -> /home " + arrivalHome + " for " + r.currentPollWorld);
            ...
            r.arrivalHomePending = false;
            AutoQiqiClient.logDebug("Legendary", "WAITING_AFTER_SWITCH: switch to " + r.currentPollWorld + " complete (waited " + waitTicks + " ticks), now polling");
    ...
    private void handlePolling(MinecraftClient client, AutoQiqiConfig config, LegendaryRunState r) {
        if (client.currentScreen != null) return;
        if (isInBattle()) return;
        AutoQiqiClient.logDebug("Legendary", "POLLING: sending " + config.nextlegCommand + " for " + r.currentPollWorld);
    ...
    private void handleWaitingForResponse(LegendaryRunState r) {
        if ((tickCount - r.stateEnteredTick) > 400) {
            AutoQiqiClient.logDebug("Legendary", "WAITING_FOR_POLL_RESPONSE: timeout (400 ticks) for " + r.currentPollWorld
                    + " (isEventRepoll=" + r.isEventRepoll + ")");
            if (r.isEventRepoll && r.eventRepollRetries < MAX_EVENT_REPOLL_RETRIES) {
                r.eventRepollRetries++;
                AutoQiqiClient.logDebug("Legendary", "Event repoll response timeout, retrying ("
    ...
    private void handleWaitingAfterEventSwitch(AutoQiqiConfig config, LegendaryRunState r) {
        ...
        if (r.pendingHomeWorld != null) {
            ...
            if (!verifyHomeDimension(mc, r.pendingHomeWorld)) {
                if (r.homeRetryCount < MAX_HOME_RETRIES) {
                    ...
                    AutoQiqiClient.logDebug("Legendary", "WAITING_AFTER_EVENT_SWITCH: dimension mismatch, retrying /home " + homeCmd);
            ...
            r.homeRetryCount = 0;
            AutoQiqiClient.logDebug("Legendary", "WAITING_AFTER_EVENT_SWITCH: /home warmup done, setting currentWorld=" + r.pendingHomeWorld);
            WorldTracker.get().setCurrentWorld(r.pendingHomeWorld);
            r.pendingHomeWorld = null;
            r.isHomeTeleport = false;
            r.stateEnteredTick = tickCount;
        ...
        if ((tickCount - r.stateEnteredTick) > 6000) {
            AutoQiqiClient.logDebug("Legendary", "WAITING_AFTER_EVENT_SWITCH: timeout (5min) for " + r.targetEventWorld + ", forcing repoll");
        ...
        if (r.eventTimerZeroTick == 0) {
            AutoQiqiClient.logDebug("Legendary", "WAITING_AFTER_EVENT_SWITCH: timer reached 0 for " + r.targetEventWorld + ", playing bell");
            r.eventTimerZeroTick = tickCount;
            MinecraftClient client = MinecraftClient.getInstance();
    ...
    private void transitionToEventRepoll(LegendaryRunState r) {
        ...
        AutoQiqiClient.logDebug("Legendary", "transitionToEventRepoll: " + r.targetEventWorld + " -> polling for new timer");
    ...
    public void onTimerResponseReceived() {
        ...
        AutoQiqiClient.logDebug("Legendary", "Timer response received for " + r.currentPollWorld
        ...
        if (r.isEventRepoll) {
            ...
            if (timerValue <= 0 && r.eventRepollRetries < MAX_EVENT_REPOLL_RETRIES) {
                r.eventRepollRetries++;
                AutoQiqiClient.logDebug("Legendary", "Event repoll: timer still 0, retry #" + r.eventRepollRetries);
                setState(r, LegendaryRunState.State.WAITING_AFTER_EVENT_SWITCH, "repoll timer=0 retry");
                r.eventTimerZeroTick = tickCount;
                return;
            }

            AutoQiqiClient.logDebug("Legendary", "Event repoll done for " + r.currentPollWorld
    ...
    private void returnToIdle(LegendaryRunState r) {
        ...
        AutoQiqiClient.logDebug("Legendary", "returnToIdle: cooldown=" + cooldownTicks + " ticks (" + reason + "). " + dumpTimerSummary());
    ...
    public void forcePoll() {
        ...
        MinecraftClient client = MinecraftClient.getInstance();
        if (client.player != null) {
            AutoQiqiClient.logDebug("Legendary", "forcePoll: marking all timers for repoll");
    ...
    public void relearnCommands() {
        LegendaryRunState r = getOrCreateRun();
        AutoQiqiClient.logDebug("Legendary", "relearnCommands: resetting GUI world map");
    ...
    public void cancelToIdle() {
        ...
        AutoQiqiClient.logDebug("Legendary", "cancelToIdle: " + run.state + " -> IDLE (tower or manual)");
        GuiWorldSwitcher.get().cancelPending();
    ...
    private void sendCommand(MinecraftClient client, String command, LegendaryRunState r) {
        ...
        AutoQiqiClient.logDebug("Legendary", "Command queued: '" + command + "' (delay=" + delayTicks + " ticks / " + delayMs + "ms)");
    }
    ...
    private void tickPendingCommand(MinecraftClient client, LegendaryRunState r) {
        if (r.pendingCommand != null && tickCount >= r.commandExecuteAtTick) {
            if (client.player == null || !AutoQiqiClient.isConnected(client)) {
                ...
                AutoQiqiClient.logDebug("Legendary", "Command deferred (no connection), retry in 2s: '" + r.pendingCommand + "'");
                return;
            }
            if (isInBattle()) {
                AutoQiqiClient.logDebug("Legendary", "Command cancelled (in battle): '" + r.pendingCommand + "' — no teleport");
                r.pendingCommand = null;
                GuiWorldSwitcher.get().cancelPending();
            ...
            AutoQiqiClient.logDebug("Legendary", "Command executing: '" + cmd + "'");
            try {
                if (cmd.startsWith("/")) {
                ...
                AutoQiqiClient.logDebug("Legendary", "Command send failed (network?): " + e.getMessage() + " — re-queuing");
    ...
    private void setState(LegendaryRunState r, LegendaryRunState.State newState, String reason) {
        if (r.state != newState) {
            AutoQiqiClient.logDebug("Legendary", "State: " + r.state + " -> " + newState + " (" + reason + ")");
        }
        r.state = newState;
    }
